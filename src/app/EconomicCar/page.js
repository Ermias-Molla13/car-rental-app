...
import { Eco } from '../../data/EcoCars.js';
import Link from 'next/link';
import { Drawer, Button } from "antd";
import { FilterOutlined } from "@ant-design/icons";
import FilterSidebar from '../components/EconomicFilterSidebar.js';
import EcoCars from '../components/EconomicCar.js';

export default function CarsPage() {
  const [currentPage, setCurrentPage] = useState(1);
  const [filterDrawerVisible, setFilterDrawerVisible] = useState(false);
  const [filteredCars, setFilteredCars] = useState(Eco);
  const carsPerPage = 8;
  
  // Calculate pagination based on filtered Eco
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
              <h1 className="text-2xl md:text-3xl font-bold text-gray-800">
                Economic Class Eco ({filteredCars.length})
              </h1>
              <Link 
                href="/" 
            ...
            {filteredCars.length > 0 ? (
              <>
                ...
                <div className="mt-4 text-center text-gray-600">
                  Showing Eco {indexOfFirstCar + 1}-{Math.min(indexOfLastCar, filteredCars.length)} of {filteredCars.length}
                </div>
              </>
            ) : (
